...
export default function TeacherDashboard() {
  ...
  const handleAvaliar = async (e) => {
    ...
    try {
      ...
      carregarProjetos(); // recarrega para exibir a nota recém-registrada
    } catch (err) {
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-2 ml-4 flex-shrink-0">
                        ...
                        <button
                          ...
                          className="text-xs px-3 py-1.5 rounded-lg font-medium hover:opacity-80 transition"
                        >
                          {avaliando === p.id ? 'Cancelar' : jaAvaliado ? 'Reavaliar' : 'Avaliar'}
                        </button>
                      </div>
                    </div>
                      ...
                      <form onSubmit={handleAvaliar} className="mt-4 pt-4 border-t border-gray-100">
                        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                          <div>
                            ...
                            {form.nota && (
                              <span
                                style={{ background: statusInfo(form.nota).bg, color: statusInfo(form.nota).color }}
                                className="inline-block mt-1 text-xs font-medium px-2 py-0.5 rounded-full"
                              >
                                {statusInfo(form.nota).label}
                              </span>
                            )}
                          </div>
