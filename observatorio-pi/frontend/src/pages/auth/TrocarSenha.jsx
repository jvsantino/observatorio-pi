...
export default function TrocarSenha() {
  ...
  const handleTrocar = async (e) => {
    ...
    if (novaSenha === 'Trocar@123') { setErro('Escolha uma senha diferente da senha padrão.'); return; }
    ...
    try {
      ...
      // Marca no banco que a senha já foi trocada
      await api.post('/auth/senha-trocada');

      // Reload forçado: o AuthContext relê /auth/me (já sem a obrigação) e o loop não acontece
      const rotas = { aluno: '/student', professor: '/teacher', administrador: '/admin', coordenador: '/admin', empresa: '/empresa' };
      window.location.href = rotas[user?.role] || '/';
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center px-6" style={{ background: '#f0f4f8' }}>
      <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-sm">
        <div className="text-center mb-8">
          <div style={{ background: '#F7941C' }} className="w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-xl mx-auto mb-3">🔒</div>
